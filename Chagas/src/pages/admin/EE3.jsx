...
export default function EE3() {
  ...
  const pct = (num, den) => Number(den) > 0 ? `${Math.round((Number(num)/Number(den))*100)}%` : "0%";
  ...
  const tot = evaluaciones.reduce((a, it) => {
    ...
    a.est_exist += Number(getFirst(it, ["est_ae_exist"], 0)) || 0;
    a.est_ejec  += Number(getFirst(it, ["est_ae_ejec"], 0)) || 0;

    a.com_exist += Number(getFirst(it, ["com_ae_exist"], 0)) || 0;
    a.com_ejec  += Number(getFirst(it, ["com_ae_ejec"], 0)) || 0;
    ...
    est_exist:0, est_ejec:0, com_exist:0, com_ejec:0,
  ...
  const coberturaViv = pct(tot.viv_eval, tot.viv_exist);
              ...
              <table className="data-table wide">
                <thead>
                  <tr>
                    ...
                    <th colSpan="3">N° EST SALUD ÁREA ENDÉMICA</th>
                    <th colSpan="3">N° COMUNIDADES ÁREA ENDÉMICA</th>
                  ...
                  <tr>
                    ...
                    <th rowSpan="2">Total Exist</th>
                    <th rowSpan="2">Total Ejec</th>
                    <th rowSpan="2">% Cob</th>
                    ...
                    <th rowSpan="2">Exist.ÁreaEndémica</th>
                ...
                <tbody>
                  {evaluaciones.length > 0 ? (
                    evaluaciones.map((it, idx) => {
                      ...
                      const estExist = getFirst(it, ["est_ae_exist"], 0);
                      const estEjec  = getFirst(it, ["est_ae_ejec"], 0);
                      const estCob   = getFirst(it, ["est_ae_cob"], pct(estEjec, estExist));

                      const comExist = getFirst(it, ["com_ae_exist"], 0);
                      const comEjec  = getFirst(it, ["com_ae_ejec"], 0);
                      const comCob   = getFirst(it, ["com_ae_cob"], pct(comEjec, comExist));

                      return (
                        <tr key={it.id || idx}>
                          <td>{idx + 1}</td>
                          <td className="text-bold">{it.municipio_nombre || it.municipio || "N/A"}</td>
                          ...
                          <td className="text-center">{estExist}</td>
                          <td className="text-center">{estEjec}</td>
                          <td className="text-center">{estCob}</td>

                          <td className="text-center">{comExist}</td>
                          <td className="text-center">{comEjec}</td>
                          <td className="text-center">{comCob}</td>
                          ...
                          <td className="text-center">{getFirst(it, ["porc_cobertura"], pct(getFirst(it,["viviendas_revisadas"],0), getFirst(it,["viviendas_existentes"],0)))}</td>

                          <td className="text-center">{getFirst(it, ["viv_positivas"], 0)}</td>
                          <td className="text-center">{getFirst(it, ["porc_iv"], pct(getFirst(it,["viv_positivas"],0), getFirst(it,["viviendas_revisadas"],0)))}</td>
                          <td className="text-center">{getFirst(it, ["viv_pos_intra"], 0)} ({getFirst(it, ["porc_iii"], pct(getFirst(it,["viv_pos_intra"],0), getFirst(it,["viviendas_revisadas"],0)))})</td>
                          <td className="text-center">{getFirst(it, ["viv_pos_peri"], 0)} ({getFirst(it, ["porc_ip"], pct(getFirst(it,["viv_pos_peri"],0), getFirst(it,["viviendas_revisadas"],0)))})</td>

                          <td className="text-center">{getFirst(it, ["viv_con_ninfas"], 0)}</td>
                          <td className="text-center">{getFirst(it, ["porc_iic"], pct(getFirst(it,["viv_con_ninfas"],0), getFirst(it,["viviendas_revisadas"],0)))}</td>
                          <td className="text-center">{getFirst(it, ["viv_ci_intra"], 0)} ({getFirst(it, ["porc_ci"], pct(getFirst(it,["viv_ci_intra"],0), getFirst(it,["viviendas_revisadas"],0)))})</td>
                          <td className="text-center">{getFirst(it, ["viv_ci_peri"], 0)} ({getFirst(it, ["porc_icp"], pct(getFirst(it,["viv_ci_peri"],0), getFirst(it,["viviendas_revisadas"],0)))})</td>
                          ...
                          <td className="text-center">
                            <BotonVerMapa
                              ...
                              comunidad={it.municipio || "Ubicación"}
                            />
                          </td>
                        </tr>
                      );
                ...
                {evaluaciones.length > 0 && (
                  <tfoot>
                    <tr className="table-totals">
                      ...
                      <td className="text-center text-bold">{tot.est_exist}</td>
                      <td className="text-center text-bold">{tot.est_ejec}</td>
                      <td className="text-center text-bold">{pct(tot.est_ejec, tot.est_exist)}</td>

                      <td className="text-center text-bold">{tot.com_exist}</td>
                      <td className="text-center text-bold">{tot.com_ejec}</td>
                      <td className="text-center text-bold">{pct(tot.com_ejec, tot.com_exist)}</td>
                      ...
                      <td className="text-center text-bold">{coberturaViv}</td>
