// App.js - VERSIÓN FINAL UNIFICADA
import React from 'react';
import { BrowserRouter as Router, Routes, Route } from 'react-router-dom';
import { AuthProvider } from './pages/AuthContext';
import NavBar from './pages/NavBar';
import Footer from './pages/Footer';
import Register from './pages/Register';
import AboutUs from './pages/AboutUs';
import Login from './pages/Login';
import Home from './pages/Home';

/* ========== ADMIN ========== */
import RR1 from './pages/admin/RR1';
import RR2 from './pages/admin/RR2';
import RR3 from './pages/admin/RR3';
import EE1 from './pages/admin/EE1';
import EE2 from './pages/admin/EE2';
import EE3 from './pages/admin/EE3';
import Manejo from './pages/admin/Manejo';
import DetalleEE1 from './pages/DatosEE1';

/* ========== USUARIOS ========== */
import Usuarios from './pages/Usuarios/usuarios';
import UsuarioView from './pages/Usuarios/usuarioView';
import UsuarioEdit from './pages/Usuarios/usuarioEdit';
import UsuarioCreate from './pages/Usuarios/usuarioCreate';

/* ========== FUNCIONES Y REPORTES ========== */
import Programar from './pages/Programar';
import CargaRociado from './pages/CargaRociado';
import Estadisticas from './pages/Estadisticas';
import EstadisticasNivo from './pages/EstadisticasNivo';

/* ========== DENUNCIAS ========== */
import Denuncias from './pages/Denuncias';
import RegistrarDenuncia from './pages/RegistrarDenuncia';
import DetallesDenuncia from './pages/DetallesDenuncia';

/* ========== MAPAS ========== */
import Mapas from './pages/Mapas';             // Mapa general del equipo
import MapasEE1 from './pages/MapasEE1';       // Mapa específico EE1
import MapasGeneral from './pages/MapasGeneral'; // Mapa general nuevo

/* ========== COMPONENTES DE SEGURIDAD ========== */
import ProtectedRoute from './components/ProtectedRoute';

import './App.css';

function App() {
  return (
    <AuthProvider>
      <Router>
        <div className="app-container">
          <NavBar />
          <main className="main-content">
            <Routes>
              {/* RUTAS PRINCIPALES */}
              <Route path="/" element={<Home />} />
              <Route path="/home" element={<Home />} />
              <Route path="/login" element={<Login />} />
              <Route path="/register" element={<Register />} />
              <Route path="/sobreNosotros" element={<AboutUs />} />

              {/* ADMIN */}
              <Route path="/admin/RR1" element={<RR1 />} />
              <Route path="/admin/RR2" element={<RR2 />} />
              <Route path="/admin/RR3" element={<RR3 />} />
              <Route path="/admin/EE1" element={<EE1 />} />
              <Route path="/admin/EE2" element={<EE2 />} />
              <Route path="/admin/EE3" element={<EE3 />} />
              <Route path="/admin/manejo" element={<Manejo />} />
              <Route path="/DatosEE1" element={<DetalleEE1 />} />

              {/* RUTAS DE USUARIOS PROTEGIDAS */}
              <Route
                path="/usuarios"
                element={
                  <ProtectedRoute>
                    <Usuarios />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/usuarios/view/:id"
                element={
                  <ProtectedRoute>
                    <UsuarioView />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/usuarios/edit/:id"
                element={
                  <ProtectedRoute>
                    <UsuarioEdit />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/usuarios/create"
                element={
                  <ProtectedRoute>
                    <UsuarioCreate />
                  </ProtectedRoute>
                }
              />

              {/* FUNCIONES Y ROCIADO */}
              <Route
                path="/cargaRociado"
                element={
                  <ProtectedRoute>
                    <CargaRociado />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/programar/:id"
                element={
                  <ProtectedRoute>
                    <Programar />
                  </ProtectedRoute>
                }
              />

              {/* MAPAS */}
              <Route
                path="/mapas"
                element={
                  <ProtectedRoute>
                    <Mapas />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/mapaEE1"
                element={
                  <ProtectedRoute>
                    <MapasEE1 />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/mapaGeneral"
                element={
                  <ProtectedRoute>
                    <MapasGeneral />
                  </ProtectedRoute>
                }
              />

              {/* ESTADÍSTICAS */}
              <Route
                path="/estadisticas"
                element={
                  <ProtectedRoute>
                    <Estadisticas />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/estadisticas-avanzadas"
                element={
                  <ProtectedRoute>
                    <EstadisticasNivo />
                  </ProtectedRoute>
                }
              />

              {/* DENUNCIAS */}
              <Route
                path="/denuncia"
                element={
                  <ProtectedRoute>
                    <Denuncias />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/registrar-denuncia"
                element={
                  <ProtectedRoute>
                    <RegistrarDenuncia />
                  </ProtectedRoute>
                }
              />
              <Route
                path="/detalles-denuncia/:id"
                element={
                  <ProtectedRoute>
                    <DetallesDenuncia />
                  </ProtectedRoute>
                }
              />
            </Routes>
          </main>
          <Footer />
        </div>
      </Router>
    </AuthProvider>
  );
}

export default App;
